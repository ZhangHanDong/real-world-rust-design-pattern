# Summary

# Creational Patterns (创建型模式)

- [Factory Method（工厂方法）](./factory_method.md)
- [abstract_factory（抽象工厂）](./abstract_factory.md)
- [builder（构建）](./builder.md)
- [protoype（原型）](./protoype.md)
- [singleton（单例）](./singleton.md)

# Structural Patterns (结构型模式)

- [Adapter（适配器）](./adapter.md)
- [Bridge（桥接）](./bridge.md)
- [Composite（组合）](./composite.md)
- [Decorator（装饰）](./decorator.md)
- [Facade（外观）](./facade.md)
- [Flyweight（享元）](./flyweight.md)
- [Proxy（代理）](./proxy.md)

# Behavioral Patterns (行为模式)

- [Chain of Responsibility（责任链）](./chain-of-responsibility.md)
- [command（命令）](./command.md)
- [Iterator（迭代器）](./iterator.md)
- [Mediator（中介者）](./mediator)
- [Memento（备忘录）](./memento.md)
- [Observer（观察者）](./observer.md)
- [State（状态）](./state.md)
- [Strategy（策略）](./strategy.md)
- [Template Method（模版方法）](./template_method.md)
- [Visitor（访问者）](./visitor.md)

# Rust Specific (Rust 专用)

- [Enum](./enum.md)
- [Ownership](./ownership.md)